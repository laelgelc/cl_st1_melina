#!/usr/bin/env python3
"""
Compute group-specific key lemmas for the tagged NOW corpus.
...
Each group folder should contain TreeTagger output files in .txt format.
...
cutoff = minimum percent presence requirement in the target group.
"""
...
GROUP_FOLDER_RE = re.compile(r"^global_(north|south)_\d{4}_\d{2}$")
...
def discover_group_folders(base_dir):
    """Return group-named subdirectories under the tagged corpus directory."""
    ...
    if not folders:
        raise FileNotFoundError(
            f"No group folders found in {base_dir}. "
            "Expected folders such as global_north_2023_09 or global_south_2024_10."
        )
...
def load_lemma_presence(base_dir, *, label_prefix=""):
    """
    Load lemma presence for one corpus group folder.
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute group-specific key lemmas for the tagged NOW corpus."
    )
    parser.add_argument(
        "--input",
        default="corpus/05_tagged",
        help="Directory containing tagged group folders.",
    )
    parser.add_argument(
        "--output",
        default="corpus/06_keylemmas",
    ...
    parser.add_argument(
        ...
        default=3.0,
        type=float,
        help="Minimum percentage presence in target group texts.",
    )
    ...
    folders = discover_group_folders(base_dir)

    print("Found group folders:")
    for folder in folders:
        print(f"  - {folder}")

    # Build global lemma presence across all groups.
